...
export const NotSoBigStuff = () => {

    const [NotSoBigStuff, setNotSoBigStuff] = useState([])

    useEffect(() => {
        fetch("http://localhost:9998/api/bigstuff")
            .then((res) => res.json())
            .then((data) => {
                setNotSoBigStuff(data.furn.filter(single => single.stuff === "Not So Big Stuff"))
                console.log(data)
            })
        console.log(NotSoBigStuff)
    }, [])
